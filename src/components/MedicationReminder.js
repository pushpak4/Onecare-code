...
import { faEdit, faTrash, faBell, faArrowLeft } from '@fortawesome/free-solid-svg-icons';
...
function MedicationReminder({ route, navigation }) {
  ...
  const handleSendReminder = (medicationId, medicationName, medicationDosage, medicationSchedule) => {
    ...
    setReminderTimeouts(prevState => ({
      ...prevState,
      [medicationId]: intervalId
    }));
    alert(`Reminder set for ${medicationName}.`);
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TouchableOpacity style={styles.backButton} onPress={() => navigation.goBack()}>
        <FontAwesomeIcon icon={faArrowLeft} size="lg" style={styles.backIcon} />
...
const styles = StyleSheet.create({
  ...
  stopReminderButton: {
    backgroundColor: '#ffcc00', // Yellow color for stop button
    padding: 10,
    borderRadius: 5,
    alignItems: 'center',
  },
